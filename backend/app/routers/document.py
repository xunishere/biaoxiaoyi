...
async def upload_multiple_files(files: list[UploadFile] = File(...)):
    """上传多个文档文件，提取文本后合并为一次分析内容。"""
    try:
        if not files:
            return FileUploadResponse(success=False, message="请至少上传一个文件")
        ...
        for file in files:
            if not FileService.is_supported_document(file.content_type):
                return FileUploadResponse(
                    success=False,
                    message=f"文件 {file.filename} 类型不支持，请上传PDF或Word文档",
                )
        ...
        if total_size > settings.max_file_size:
            ...
            return FileUploadResponse(
                success=False,
                message=f"文件总大小超过限制 ({total_mb:.1f}MB / {limit_mb:.0f}MB)",
            )
        ...
        for index, file in enumerate(files, start=1):
            file_content = await FileService.process_uploaded_file(file)
            merged_parts.append(
                f"===== 文件 {index}：{file.filename} =====\n{file_content}"
            )

        return FileUploadResponse(
            success=True,
            message=f"共 {len(files)} 个文件上传成功，已合并解析",
            file_content="\n\n".join(merged_parts),
        )
    except Exception as exc:
        logger.exception("多文件上传失败")
        return FileUploadResponse(success=False, message=f"文件处理失败: {exc}")
