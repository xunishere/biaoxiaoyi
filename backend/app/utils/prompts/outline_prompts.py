...
3. **特别处理"商务技术合并节"**：当原文出现"商务技术文件"、"商务技术资料"、"商务及技术资料"这种把商务与技术合并命名的一级标题时，必须将其**拆为两个独立 groups**：
   - "商务文件"：放入投标函、授权委托书、分包意向、符合性审查、廉洁自律承诺、声明函、报价表说明等非技术条目
   - "技术文件"：放入"评标标准相应的商务技术资料"、"项目需求的理解"、"总体设计方案"、"详细应用系统设计方案"、"项目组织实施方案"、"安全设计方案"、"技术方案"、"实施方案"、"技术规格"等技术条目
   拆分后必须确保有一个 group 的 title 含"技术"二字（这样后续生成可触发技术目录强制结构）
4. 不要把每个单独条目（投标书、报价表等）作为一个 group
5. 每个 group 的 detail_points 列出该大类下的所有二级条目
6. requirement_id 必须唯一，使用 F1、F2、F3 格式
7. 只返回 JSON，格式必须为 {"groups": [...]}，不要输出任何其他内容
