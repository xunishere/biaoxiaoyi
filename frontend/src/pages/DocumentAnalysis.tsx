...
const DocumentAnalysis: React.FC<DocumentAnalysisProps> = ({
  ...
  const uploadedFilesSize = uploadedFiles.reduce((sum, file) => sum + file.size, 0);
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (totalSize > MAX_TOTAL_UPLOAD_SIZE) {
      setMessage({
        type: 'error',
        text: `文件总大小超过限制（${formatFileSize(totalSize)} / ${formatFileSize(MAX_TOTAL_UPLOAD_SIZE)}）`,
      });
          ...
          <div className="mt-4">
            <p className="text-lg text-gray-600">
              {uploadedFiles.length > 0
                ? `已选择 ${uploadedFiles.length} 个文件，共 ${formatFileSize(uploadedFilesSize)}`
                : '点击选择文件或拖拽文件到这里'}
            </p>
            {uploadedFiles.length > 0 && (
              <div className="mt-2 text-sm text-gray-500 space-y-1">
                {uploadedFiles.map((file, index) => (
                  <p key={`${file.name}-${file.size}-${index}`}>{file.name}</p>
                ))}
              </div>
            )}
            <p className="text-sm text-gray-500 mt-2">
              支持 PDF 和 Word 文档，可多选，总大小不超过 10MB
            </p>
          </div>
